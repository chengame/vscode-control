...
export default function () {
  ...
  function getFlutterDevices() {
    return new Promise((resolve, reject) => {
      exec(`${getPreferenceValues()?.flutterPath || 'flutter'} devices --machine`, { cwd: workspace, shell: true }, (error, stdout) => {
        if (error) {
          reject(error)
  ...
  function sortTargetsByPriority(devices) {
    return devices
      ...
      .map(device => ({
        ...
        icon: device.targetPlatform.startsWith('web')
          ? "🌐"
          : device.targetPlatform.startsWith('darwin')
            ? "🖥️"
            : '❓',
...
${capsList || 'No capabilities info'}
  ...
  return (
    <List isLoading={loading} isShowingDetail={!loading && targets.length > 0}>
      {targets.length === 0 ? (
        <List.EmptyView title="No Flutter Targets Found" description="Make sure Flutter is installed and available in PATH" />
      ) : (
        <>
          {/* Supported Targets */}
            ...
            <List.Section title="Available Targets" subtitle={`${supportedTargets.length} found`}>
              {supportedTargets.map((target, index) => (
                <List.Item
                  ...
                  icon={target.icon}
                  detail={
                    <List.Item.Detail markdown={getDetailMarkdown(target)} />
                  }
                  actions={
                    <ActionPanel>
                      <Action
